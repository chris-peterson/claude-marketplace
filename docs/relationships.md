...
| scribe | Post-session | Sanitize the transcript into a retrospective committed to a team git repo |
...
flowchart LR
    ...
    Post[Post-session] --> Scribe[scribe]
...
- **scribe** captures the session transcript after the fact and publishes a sanitized retro to a team-owned git repository. It's decoupled from the inflight plugins.
